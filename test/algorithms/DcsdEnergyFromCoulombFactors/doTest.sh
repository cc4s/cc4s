...
# @CLASS=essential,ccsd,dcsd,trd
TEST_DESCRIPTION="DCSD energy from decomposed Coulomb vertex: check if the energy is within tolerance"
...
COMPARE_ENERGY=$COULOMBFACTORS_DCSD
TOLERANCE=1e-11
...
TEST_RESULT=$(
python <<EOF 
if abs(${ENERGY} - ${COMPARE_ENERGY})<${TOLERANCE} and abs(${ENERGYSLICE} - ${COMPARE_ENERGY})<${TOLERANCE} and abs(${ENERGYSLICE} - ${COMPARE_ENERGY})<${TOLERANCE}:
    print(0)
else:
    print(1)
EOF
