...
ChiAi::ChiAi(int nG_, int no_, int nv_): Chi(nG_, no_, nv_) {
  int lens[] = {nG, nv, no};
  int syms[] = {NS, NS, NS};
  gpq = new Tensor<>(3, lens, syms, *Cc4s::world,"Xgai",Cc4s::options->profile);
}

Idx_Tensor ChiAi::get(char const *stdIndexMap, char const *indexMap) {
  // NOTE: PerturbationTensor does not know about p,q,.. indices in stdIndexMap
  if (0 == strcmp(indexMap, "gai")) return (*gpq)[indexMap];
  if (0 == strcmp(indexMap, "gia")) {
    // also swap the last two indices in the indexMap
    char swappedIndexMap[] = { indexMap[0], indexMap[2], indexMap[1], 0 };
    return (*gpq)[swappedIndexMap];
  } else {
    std::stringstream stream("");
    stream << "Cannot fetch Chi tensor part " << stdIndexMap <<
      " with index names " << indexMap;
